...
impl IntoLaTeX for OrderedList {
    fn into_latex<'a>(&'a self, cfg: &LaTeXConfig, ctx: &mut LaTeXContext) -> PrettyPrint<'a> {
        ...
        for item in &self.children {
            enumerate = enumerate.append(item.into_latex(cfg, ctx)).append(newline())
        }
        enumerate.append(newline()).append("\\end{enumerate}")
    }
}

impl IntoLaTeX for OrderlessList {
    fn into_latex<'a>(&'a self, cfg: &LaTeXConfig, ctx: &mut LaTeXContext) -> PrettyPrint<'a> {
        let mut itemize = text_ref("\\begin{itemize}").append(newline());
        for item in &self.children {
            itemize = itemize.append(item.into_latex(cfg, ctx)).append(newline())
        }
        itemize.append(newline()).append("\\end{itemize}")
    }
}
